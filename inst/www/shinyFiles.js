var shinyFiles = (function() {
	...
	var formatSize = function(bytes, si) {
        /*
        This function is taken from http://stackoverflow.com/questions/10420352/converting-file-size-in-bytes-to-human-readable - Mark's answer    
        */
	...
	var sortFiles = function(modal, attribute, direction) {
        ...
        if(type == 'file') {
            ...
            var fileList = $(modal).find('.sF-dirContent');
        }
		...
		var modal = $('<div>', {id: $(button).attr('id')+'-modal'}).addClass('sF-modalContainer modal fade').css('display', 'block').append(
			$('<div>').addClass('sF-modal modal-dialog').append(
				$('<div>').addClass('modal-content').append(
            				...
            				$('<div>').addClass('sF-sort dropdown btn-group btn-group-sm').append(
        						...
        						$('<ul>').addClass('dropdown-menu').append(
        							$('<li>').addClass('sortAttr').append(
                                        $('<a>', {href: '#', text: 'Name'}).prepend($('<span>').addClass('glyphicon glyphicon-ok'))).addClass($(button).data('sort') == 'Name' ? 'selected' : '')
        						).append(
        							$('<li>').addClass('sortAttr').append(
                                        $('<a>', {href: '#', text: 'Type'}).prepend($('<span>').addClass('glyphicon glyphicon-ok'))).addClass($(button).data('sort') == 'Type' ? 'selected' : '')
        						).append(
        							$('<li>').addClass('sortAttr').append(
                                        $('<a>', {href: '#', text: 'Size'}).prepend($('<span>').addClass('glyphicon glyphicon-ok'))).addClass($(button).data('sort') == 'Size' ? 'selected' : '')
        						).append(
        							$('<li>').addClass('sortAttr').append(
                                        $('<a>', {href: '#', text: 'Created'}).prepend($('<span>').addClass('glyphicon glyphicon-ok'))).addClass($(button).data('sort') == 'Created' ? 'selected' : '')
        						).append(
        							$('<li>').addClass('sortAttr').append(
                                        $('<a>', {href: '#', text: 'Modified'}).prepend($('<span>').addClass('glyphicon glyphicon-ok'))).addClass($(button).data('sort') == 'Modified' ? 'selected' : '')
        						).append(
        							$('<li>').addClass('divider')
        						).append(
					...
					).append( $('<div>').addClass('sF-progressbar').toggle(false).append( $('<div>').addClass('sF-indicator').html("<p style='font-size:11px;color:white;'>CHARGEMENT...</p>")))
				).append(
    ...
    var progress = function (element, data){
        var modal = $(element).data('modal');
        var indicator = modal.find(".sF-indicator");
        var actualprogress = indicator.css("width");
	    //var maxprogress = 244; 
	    if(actualprogress == "0px"){
            modal.find(".sF-progressbar").toggle(true);
        }
        indicator.css("width","+="+data);   
        
    }
    
    var progressEnd = function(element){
        var modal = $(element).data('modal');
        var indicator = modal.find(".sF-indicator");
        modal.find(".sF-progressbar").toggle(false);
	...
	sF.init = function() {
        ...
        Shiny.addCustomMessageHandler('shinyFilesProgress', function(data) {
            progress($('.shinyFiles#'+data.id),data.width)
        });
        Shiny.addCustomMessageHandler('shinyFilesProgressEnd', function(data) {
            progressEnd($('.shinyFiles#'+data.id))
        });
        
        Shiny.addCustomMessageHandler('shinySaveProgress', function(data) {
            progress($('.shinySave#'+data.id),data.width)
        });
        Shiny.addCustomMessageHandler('shinySaveProgressEnd', function(data) {
            progressEnd($('.shinySave#'+data.id))
        });
        Shiny.addCustomMessageHandler('shinyDirProgress', function(data) {
            progress($('.shinyDirectories#'+data.id),data.width)
        });
        Shiny.addCustomMessageHandler('shinyDirProgressEnd', function(data) {
            progressEnd($('.shinyDirectories#'+data.id))
        });
