...
#' 
fileGetterSave <- function(roots, restrictions, filetypes, hidden=FALSE,session,id,dtoken) {
...
#     roots = c(Acceuil = '')
#     shinyDropFileSave(input, 'save', roots=roots, session=session)
#     output$rawInputValue <- renderPrint({str(input$save)})
#     output$filepaths <- renderPrint({parseDropSavePath(roots, input$save)})
# })
# 
# runApp(list(
#     ui=ui,
...
#' @export
#' 
parseDropSavePath <- function(roots, selection) {
...
server <- shinyServer(function(input, output,session) {
    roots=c(Accueil='', testR = '/test R')
    #roots = c(Accueil='')
    # shinyDropFileChoose(input, 'files', roots=roots, defaultPath = "",defaultRoot="Accueil",session = session, dtoken =token,filetypes=c('', 'xlsx'))
    # output$rawInputValue <- renderPrint({str(input$files)})
    # output$filepaths <- renderPrint({parseDropFilePaths(roots, input$files)})
    
    #roots = c(Acceuil = '')
    shinyDropFileSave(input, 'save', roots=roots, defaultRoot="Accueil",session=session,dtoken=token)
    output$rawInputValue <- renderPrint({str(input$save)})
    output$filepaths <- renderPrint({parseDropSavePath(roots, input$save)})
    
    # shinyDropDirChoose(input, 'folder', roots=roots,defaultRoot="Accueil", session = session,dtoken=token)
    # output$rawInputValue <- renderPrint({str(input$folder)})
    # output$filepaths <- renderPrint({parseDropDirPath(roots, input$folder)})
})
