...
#' 
fileGetterDir <- function(roots, restrictions, filetypes, hidden=FALSE,session,id,dtoken) {
...
#' 
traverseDropDirs <- function(tree, root, restrictions, hidden,dtoken) {
    ...
    if(length(folders)>0) {
        ...
        if(tree$expanded) {
            ...
            tree$children <- lapply(children, traverseDropDirs, root=location, restrictions=restrictions, hidden=hidden,dtoken=dtoken)
        }
        else {
            tree$children <- list()
        }
...
#' 
dirDropGetter <- function(roots, restrictions, filetypes, hidden=FALSE,session,id,dtoken) {
    if (missing(filetypes)) filetypes <- NULL
    if (missing(restrictions)) restrictions <- NULL
...
#' by the client.
#' 
dirDropCreator <- function(roots,dtoken, ...) {
...
#     roots=c(Accueil='')
#     shinyDropDirChoose(input, 'folder', roots=roots, session = session)
# })
# 
# runApp(list(
#     ui=ui,
...
shinyDropDirChoose <- function(input, id, updateFreq=20000, session=getDropSession(),
    ...
    return(observe({
        ...
        print(!identical(currentDir, newDir))
        print("!!!!!!!!!!!! newDir !!!!!!!!!!!!!!!!!!!!")
         print(newDir)
...
#' @export
#' 
parseDropDirPath <- function(roots, selection) {
    currentRoots <- if(class(roots) == 'function') roots() else roots
